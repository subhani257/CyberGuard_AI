...
*   **Database:** Supabase PostgreSQL (`decisions` table) & pgvector (`cyber_threats`).
...
*   **Logic:** Build `POST /api/agents/evaluate` endpoint:
    ...
    *   *Process:*
        ...
        7. Save results to PostgreSQL `decisions` table (evaluation, is_safe, human_review_required)
...
You'll work with the `decisions` table:

```sql
-- Decisions table (stores both user response and evaluation results)
CREATE TABLE decisions (
    ...
    evaluation JSONB,  -- Your evaluation JSON goes here
    is_safe BOOLEAN,
...
**Your evaluation JSON structure:**
```json
{
  "action_score": 85,
  "reasoning_score": 70,
  "final_score": 79,
  "threat_indicators": ["urgency", "financial_request"],
  "reasoning_category": "security-aware",
  "llm_evaluation": {
    "confidence": 85,
    "explanation": "User showed good security awareness..."
  }
}
```
